...
import { HABIT_STATUS, type Habit, type HabitStatus } from '../model/types';
import type { HabitKanbanBoardState, HabitKanbanItem } from './types';

//* Local State Utils
export function buildHabitBoard(habits: Habit[]): HabitKanbanBoardState {
...
export function applyDragToBoard(
  board: HabitKanbanBoardState,
  result: DropResult,
): HabitKanbanBoardState {
  if (!hasValidDestination(result)) return board;

  const { source, destination } = result;
  if (!destination || isSameLocation(source, destination)) return board;
  ...
  return normalizePositions(updatedBoard);
}

//* Server utils

export function buildReorderDiffPayload(
  prevBoard: HabitKanbanBoardState,
  nextBoard: HabitKanbanBoardState,
) {
  const prevById = new Map<string, { status: HabitStatus; position: number }>();
